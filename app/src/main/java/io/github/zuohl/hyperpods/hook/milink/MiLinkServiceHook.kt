package io.github.zuohl.hyperpods.hook.milink
...
import io.github.zuohl.hyperpods.hook.HookContext
import io.github.zuohl.hyperpods.hook.Log
import io.github.zuohl.hyperpods.hook.callMethod
import io.github.zuohl.hyperpods.hook.getObjectField
import io.github.zuohl.hyperpods.hook.setObjectField
...
object MiLinkServiceHook : HookContext() {
    ...
    internal fun hookStringAddressResult(className: String, methodName: String, result: () -> Any) {
        runCatching {
            hookAfter(findMethod(className, methodName, String::class.java)) {
                ...
                if (!isKnownAddress(address)) return@hookAfter
                this.result = result()
            }
    ...
    private fun hookAncStateBlock() {
        runCatching {
            hookBefore(findMethod("com.miui.headset.runtime.AncBatteryController", "setAncStateBlock", BluetoothDevice::class.java, Int::class.javaPrimitiveType!!)) {
                ...
                val oppoAnc = podAncFromMiLink(miLinkMode)
                ...
                currentAnc = oppoAnc
                sendPodAnc(oppoAnc, instanceContext)
    ...
    private fun registerStatusReceiver(ctx: Context?) {
        ...
        context?.sendBroadcast(Intent(PodAction.ACTION_PODS_UI_INIT).apply {
    ...
    internal fun isKnownAddress(address: String): Boolean {
    ...
    private fun podAncFromMiLink(mode: Int): Int {
        return when (mode) {
            1 -> 2
            2 -> 3
            else -> 1
    ...
    private fun chargingValue(params: io.github.zuohl.hyperpods.utils.miuiStrongToast.data.PodParams?): Int {
        return if (params?.isConnected == true && params.isCharging) 1 else 0
    }

    private fun sendPodAnc(mode: Int, fallbackContext: Context? = null) {
    ...
    internal fun sendPodSpatialAudio(mode: Int, fallbackContext: Context? = null) {
        val ctx = fallbackContext ?: context ?: run {
            Log.w(TAG, "sendPodSpatialAudio skipped: context is null mode=$mode")
            return
        }
    ...
    internal fun sendSpatialChanged(mode: Int, fallbackContext: Context? = null) {
        ...
        listOf(BuildConfig.APPLICATION_ID, "com.milink.service", "com.android.settings").forEach { targetPackage ->
            ctx.sendBroadcast(Intent(PodAction.ACTION_PODS_SPATIAL_AUDIO_CHANGED).apply {
