...
fun RfcommDebugPage(
    ...
    DisposableEffect(context) {
        val receiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                if (intent?.action != PodAction.ACTION_RFCOMM_LOG) return
                logs.add(
                    RfcommDebugLogEntry(
        ...
        context.registerReceiver(receiver, IntentFilter(PodAction.ACTION_RFCOMM_LOG), Context.RECEIVER_EXPORTED)
        context.sendRfcommDebugBroadcast(PodAction.ACTION_RFCOMM_LOG_CONNECT)
        onDispose {
            context.sendRfcommDebugBroadcast(PodAction.ACTION_RFCOMM_LOG_DISCONNECT)
    ...
    LaunchedEffect(clearRequest) {
        if (clearRequest > 0) {
            logs.clear()
            context.sendRfcommDebugBroadcast(PodAction.ACTION_RFCOMM_LOG_CLEAR)
        }
    }
    ...
    Column(
        ...
        Row(
            ...
            TextButton(
                text = "发送",
                onClick = {
                    context.sendRfcommDebugBroadcast(PodAction.ACTION_RFCOMM_DEBUG_SEND) {
