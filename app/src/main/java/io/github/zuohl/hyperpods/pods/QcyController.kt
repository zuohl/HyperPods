...
object QcyController {
    ...
    private var currentSpatialAudioMode: Int = ConfigManager.SPATIAL_AUDIO_OFF
    ...
    private val broadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            when (intent.action) {
                ...
                PodAction.ACTION_REFRESH_STATUS -> queryStatus()
                ...
                PodAction.ACTION_CUSTOM_EQ_SET -> {
    ...
    fun disconnectedPod(context: Context, device: BluetoothDevice) {
        ...
        currentSpatialAudioMode = ConfigManager.SPATIAL_AUDIO_OFF
        ...
        sendAppStatusBroadcast(PodAction.ACTION_PODS_DISCONNECTED, force = true) {
    ...
    private fun handleBatteryChanged(batteryParams: BatteryParams) {
        ...
        if (!showedConnectedToast) {
            ...
            if (ctx != null && device != null) {
                MiuiStrongToastUtil.showPodsBatteryToastByMiuiBt(ctx, batteryParams, device)
            }
        }
    ...
    private fun changeUILdacStatus(enabled: Boolean) {
        sendAppStatusBroadcast(PodAction.ACTION_PODS_LDAC_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUIDynamicEqStatus(enabled: Boolean) {
        sendAppStatusBroadcast(PodAction.ACTION_PODS_DYNAMIC_EQ_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUISleepModeStatus(enabled: Boolean) {
        sendAppStatusBroadcast(PodAction.ACTION_PODS_SLEEP_MODE_CHANGED) {
            putExtra("enabled", enabled)
        }
    }

    private fun changeUIAdaptiveVolumeStatus(enabled: Boolean) {
        sendAppStatusBroadcast(PodAction.ACTION_PODS_ADAPTIVE_VOLUME_CHANGED) {
            putExtra("enabled", enabled)
        }
    }
    ...
    private fun changeUICustomEqGains(gains: IntArray) {
        sendAppStatusBroadcast(PodAction.ACTION_PODS_CUSTOM_EQ_CHANGED) {
    ...
    private fun registerReceiverIfNeeded(context: Context) {
        ...
        context.registerReceiver(
            ...
            IntentFilter().apply {
                ...
                addAction(PodAction.ACTION_ANC_SELECT)
                addAction(PodAction.ACTION_GAME_MODE_SET)
                addAction(PodAction.ACTION_SPATIAL_AUDIO_SET)
                addAction(PodAction.ACTION_DUAL_DEVICE_CONNECTION_SET)
                addAction(PodAction.ACTION_LDAC_SET)
                addAction(PodAction.ACTION_DYNAMIC_EQ_SET)
                addAction(PodAction.ACTION_SLEEP_MODE_SET)
                addAction(PodAction.ACTION_ADAPTIVE_VOLUME_SET)
                addAction(PodAction.ACTION_EQ_PRESET_SET)
                addAction(PodAction.ACTION_CUSTOM_EQ_SET)
            },
