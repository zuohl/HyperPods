...
import io.github.zuohl.hyperpods.utils.PodImageLoader
    ...
    override fun onHook() {
        ...
        fun createPodsNotification(bluetoothDevice: BluetoothDevice?, context: Context, batteryParams: BatteryParams) {
            ...
            try {
                ...
                val ancCycleIntent = Intent(PodAction.ACTION_CYCLE_ANC)
                ...
                val pendingIntent = PendingIntent.getActivity(
                    context,
                    0,
                    Intent("io.github.zuohl.hyperpods.action.show_pods_ui").apply {
                        setClassName(BuildConfig.APPLICATION_ID, "io.github.zuohl.hyperpods.PopupActivity")
            ...
            val context = getObjectField(instance, "mContext") as Context
                    ...
                    val broadcastReceiver = object : BroadcastReceiver() {
                        override fun onReceive(p0: Context?, p1: Intent?) {
                            if (p1?.action == "io.github.zuohl.hyperpods.action.sendstrongtoast") {
                                if (ConfigManager.islandMode() != ConfigManager.ISLAND_MODE_MODULE) {
                                    Log.d("HyperPods", "skip module island mode=${ConfigManager.islandMode()}")
                                    return
                                }
                            ...
                            } else if (p1?.action == "io.github.zuohl.hyperpods.action.updatepodsnotification") {
                    ...
                    val intentFilter = IntentFilter("io.github.zuohl.hyperpods.action.sendstrongtoast")
                    intentFilter.addAction("io.github.zuohl.hyperpods.action.updatepodsnotification")
                    intentFilter.addAction("io.github.zuohl.hyperpods.action.cancelpodsnotification")
                    intentFilter.addAction(PodAction.ACTION_CYCLE_ANC)
                    // 监听耳机实际 ANC 状态变更广播，保持 localAncMode 与 RfcommController 同步
                    intentFilter.addAction(PodAction.ACTION_PODS_ANC_CHANGED)
